...
const getIconForLine = (line: string) => {
  ...
  if (
    ...
    lowerLine.includes("awesome") ||
    lowerLine.includes("love") ||
    lowerLine.includes("support") ||
    lowerLine.includes("together") ||
  ...
  if (
    lowerLine.includes("congrats") ||
    lowerLine.includes("celebrate") ||
  ...
  if (
    ...
    lowerLine.includes("nice") ||
    lowerLine.includes("good job")
  ) {
    return ThumbsUp;
  }

  if (
    lowerLine.includes("exciting") ||
    lowerLine.includes("amazing") ||
    lowerLine.includes("wow") ||
    lowerLine.includes("let's go") ||
  ...
  if (
    ...
    lowerLine.includes("go ahead") ||
    lowerLine.includes("jump in")
  ) {
    return Zap;
  }

  if (
    lowerLine.includes("hey") ||
    lowerLine.includes("hi") ||
    lowerLine.includes("welcome") ||
    lowerLine.includes("glad") ||
    lowerLine.includes("nice to") ||
    lowerLine.includes("relax")
  ) {
    return Smile;
  }
  ...
  if (
    lowerLine.includes("?") ||
    lowerLine.includes("wonder") ||
    lowerLine.includes("curious") ||
    lowerLine.includes("explore") ||
    lowerLine.includes("find out")
  ) {
    return HelpCircle;
  }
  ...
  if (
    ...
    lowerLine.includes("reflect") ||
    lowerLine.includes("consider") ||
    lowerLine.includes("understand") ||
    lowerLine.includes("pause") ||
    lowerLine.includes("moment")
  ) {
    return Brain;
  }

  if (
    lowerLine.includes("learn") ||
    lowerLine.includes("know") ||
    lowerLine.includes("discover") ||
    lowerLine.includes("understand") ||
    lowerLine.includes("concept")
  ) {
    return BookOpen;
  }
  ...
  if (
    ...
    lowerLine.includes("realize") ||
    lowerLine.includes("see") ||
    lowerLine.includes("notice") ||
    lowerLine.includes("aha")
  ) {
    return Lightbulb;
  }

  if (
    lowerLine.includes("careful") ||
    lowerLine.includes("watch out") ||
  ...
  if (
    lowerLine.includes("chat") ||
    lowerLine.includes("talk") ||
    lowerLine.includes("discuss") ||
    lowerLine.includes("share") ||
    lowerLine.includes("tell me")
  ) {
    return MessageCircle;
  }

  if (
    lowerLine.includes("fun") ||
  ...
  if (
    lowerLine.includes("easy") ||
    lowerLine.includes("comfortable") ||
        ...
        <div className="space-y-4">
          <InLessonQuestion
            ...
            onAnswer={(optionId: string) =>
              onInlessonAnswer?.(questionId, optionId)
            }
  ...
  if (messageType === "inlesson_feedback" && role === "assistant") {
    ...
    return (
      <div className="space-y-3">
        ...
        {feedbackLine && (
          <p
            className={`font-bold text-sm ${feedback.type === "correct" ? "text-emerald-600" : "text-red-600"}`}
          >
        ...
        <div className="space-y-4">
          ...
          {parsed.questions.map((question) => (
            <AssessmentQuestion
              ...
              onAnswer={(answer) =>
                onQuestionAnswer?.(question.questionNumber, answer)
              }
    ...
    <div className="text-sm leading-relaxed">
      {content.split("\n").map((line, index) => {
        ...
        if (isHorizontalRule(line)) {
          return <hr className="my-3 border-gray-200" key={index} />;
        }
        ...
        if (headerMatch) {
          ...
          return (
            <p className={headerStyles[headerMatch.level]} key={index}>
              {parseInlineMarkdownWithTimestamps(
                headerMatch.text,
                onTimestampClick
              )}
        ...
        if (listItem) {
          if (listItem.type === "numbered") {
            ...
            return (
              <div className="my-1 flex items-start gap-2" key={index}>
                <span className="min-w-6 font-semibold text-gray-700">
                  {listItem.number}.
                </span>
                <span>
                  {parseInlineMarkdownWithTimestamps(
          ...
          if (hasTimestampLinks(listItem.content)) {
            return (
              <div
                className={`my-1 ${listItem.isIndented ? "ml-10" : "ml-6"}`}
                key={index}
              >
                {parseInlineMarkdownWithTimestamps(
                  listItem.content,
          ...
          const Icon =
            role === "assistant" ? getIconForLine(listItem.content) : Sparkles;

          return (
            <div
              className={`my-1 flex items-start gap-2 ${listItem.isIndented ? "ml-6" : "ml-2"}`}
              key={index}
            >
              <Icon
                className={`mt-0.5 h-4 w-4 shrink-0 ${role === "assistant" ? "text-blue-500" : "text-gray-400"}`}
              />
              <span>
                {parseInlineMarkdownWithTimestamps(
                  listItem.content,
        ...
        if (isLearningHeader(line)) {
          return (
            <p className="mt-3 mb-1 font-semibold text-gray-900" key={index}>
              {parseInlineMarkdownWithTimestamps(line, onTimestampClick)}
            </p>
          );
        }
